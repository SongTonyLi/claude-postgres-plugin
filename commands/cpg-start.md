...
First, check whether a compiled binary exists at `$CLAUDE_PLUGIN_ROOT/bin/cpg`. If it does, prefer it (no Bun runtime needed):

```bash
"$CLAUDE_PLUGIN_ROOT/bin/cpg" start
```
...
- A reminder that the same local SQLite database powers both the dashboard and the `claude-postgres` MCP server, so any conversation Claude can search is also browsable in the UI
